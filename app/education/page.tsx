        ...
        {tab === "SKILL" && (
          <div className="space-y-3">
            {skills.length === 0 ? (
              ...
              skills.map((e) => {
                const status = (e.metadata.status as string) || "IN_PROGRESS";
                return (
                  <div key={e.id} className="bg-white rounded-xl border border-gray-100 p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[status] || "bg-gray-100 text-gray-500"}`}>
                        {status === "DONE" ? "Đã xong" : "Đang học"}
                      </span>
                    </div>
                    {e.description && (
        ...
        {tab === "BOOK" && (
          <div className="space-y-3">
            {books.length === 0 ? (
              ...
              books.map((e) => (
                <div key={e.id} className="bg-white rounded-xl border border-gray-100 p-4 flex gap-3">
                  {e.images[0] ? (
                ...
                <div key={e.id} className="bg-white rounded-xl border border-gray-100 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Từ {formatDate(e.date)}</p>
                    </div>
                  </div>
                  {e.description && (
                    <p className="text-xs text-gray-500 mt-2 leading-relaxed">{e.description}</p>
